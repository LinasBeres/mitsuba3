...
public:
    ...
    std::pair<Spectrum, Bool> sample(const Scene *scene, Sampler *sampler,
        ...
        for (int s = 0; s <= n_sensor; ++s) {
            for (int e = 0; e <= n_emitter; ++e) {
                int depth = s + e;
                if ((s == 0 && e == 0) || depth <= 0 ||
                    depth > (int) m_max_depth)
                    continue;
                Spectrum L = connect_paths(scene, sampler, sensor_paths,
                                           emitter_paths, s, e);
    ...
    Spectrum connect_paths(const Scene *scene, Sampler *sampler,
                           const std::vector<Vertex> &sensor_paths,
                           const std::vector<Vertex> &emitter_paths, int s,
                           int e) const {

        Spectrum result = 0.f;

        if (e == 0) {
            // Interpret the sensor subpath as a complete path, i.e. direct
            // emission.
            Vertex prev_vertex =
                sensor_paths[s - 1];         // get throughput from previous hit
            ...
            if (dr::any_or<true>(active_em)) {
                ...
                auto [bsdf_val, bsdf_pdf] =
                    vertex.bsdf->eval_pdf(bsdf_ctx, vertex.si, wo);

                bsdf_val =
                    vertex.si.to_world_mueller(bsdf_val, -wo, vertex.si.wi);
                ...
                result[active_em] =
                    vertex.throughput * bsdf_val * em_weight * mis_em;
            }
        } else {
            ...
            Mask active = has_flag(s_vertex.bsdf->flags(), BSDFFlags::Smooth) &&
                          has_flag(e_vertex.bsdf->flags(), BSDFFlags::Smooth);
            if (dr::none(active))
                return result;

            Mask occluded = scene->ray_test(
                s_vertex.si.spawn_ray_to(e_vertex.si.p), active);
            if (dr::none(occluded)) {
                // Prep direction.
                ...
                BSDFContext bsdf_ctx;
                auto [sensor_bsdf_val, sensor_bsdf_pdf] =
                    s_vertex.bsdf->eval_pdf(bsdf_ctx, s_vertex.si, wo);

                // Eval emitter vertex bsdf
                Vector3f wi = e_vertex.si.to_local(-d);
                bsdf_ctx    = BSDFContext(TransportMode::Importance);
                auto [emitter_bsdf_val, emitter_bsdf_pdf] =
                    e_vertex.bsdf->eval_pdf(bsdf_ctx, e_vertex.si, wi);

                Float mis_bsdf = mis_weight(sensor_bsdf_pdf, emitter_bsdf_pdf);

                result = s_vertex.throughput * sensor_bsdf_val *
                         e_vertex.throughput * emitter_bsdf_val * mis_bsdf *
                         dr::square(inv_dist);
            }
        }
    ...
    int random_walk(const Scene *scene, Sampler *sampler, const Ray3f &ray_,
                    Spectrum throughput, int max_depth,
                    std::vector<Vertex> &path, TransportMode transport_mode,
                    Bool active) const {
        ...
        struct LoopState {
            ...
            DRJIT_STRUCT(LoopState, ray, throughput, bounces, active, sampler)
        } ls = { ray_, throughput,
                 /* bounces: */ 0, active, sampler };
        ...
        dr::tie(ls) = dr::while_loop(
            ...
            [scene, bsdf_ctx, max_depth, transport_mode, &path](LoopState &ls) {
                ...
                Bool active_next =
                    ls.active && (ls.bounces + 1 < max_depth) && si.is_valid();
                ...
                if (transport_mode == TransportMode::Importance) {
                    ...
                    Float wi_dot_geo_n = dr::dot(si.n, -ls.ray.d),
                          wo_dot_geo_n =
                              dr::dot(si.n, si.to_world(bsdf_sample.wo));

                    // Prevent light leaks due to shading normals
                    ls.active &=
                        (wi_dot_geo_n * Frame3f::cos_theta(si.wi) > 0.f) &&
                        (wo_dot_geo_n * Frame3f::cos_theta(bsdf_sample.wo) >
                         0.f);
